...
def generate_sample_data():
    ...
    print("生成w权重系数矩阵...")
    w_data = pd.DataFrame(
        np.random.normal(0, 0.1, (len(stations), len(water_params))),
        index=stations,
        columns=water_params
    )
    ...
    print("生成a权重系数矩阵...")
    a_data = pd.DataFrame(
        np.random.normal(0, 0.05, (len(stations), len(water_params))),
        index=stations,
        columns=water_params
    )
    ...
    print("生成b幂系数矩阵...")
    b_data = pd.DataFrame(
        np.random.normal(1, 0.2, (len(water_params), len(stations))),
        index=water_params,
        columns=stations
    )
    ...
    print("生成A微调系数...")
    A_data = pd.DataFrame(
        np.random.normal(1, 0.1, (len(water_params), 1)),
        index=water_params,
        columns=["A"]
    )
    A_data.to_csv(sample_data_dir / "A_coefficients.csv")
