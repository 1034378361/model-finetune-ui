...
# 导入本地工具
from .utils import EncodingDetector, FileUtils, performance_monitor, EnhancedLogger
...
class FileHandler:
    ...
    def read_uploaded_file(self, uploaded_file, file_type: str) -> Optional[pd.DataFrame]:
        """
        读取上传的文件
        ...
        try:
            ...
            if df is not None:
                ...
                logger.info(f"成功读取{file_type}文件: {uploaded_file.name}, 形状: {df.shape}")
    ...
    def validate_file_structure(self, df: pd.DataFrame, file_type: str) -> Tuple[bool, List[str]]:
        """
        验证文件结构
        ...
        try:
            ...
            if file_type in ['w', 'a', 'b']:
                ...
                # Range数据应该有多行观测值
                if df.shape[0] < 2:
                    errors.append(f"Range数据应该有多行观测值用于计算范围")
            ...
            if file_type != 'Range':
                # 检查是否有预期的水质参数
                matching_params = [param for param in expected_params if param in df.index]
    ...
    def get_file_preview(self, df: pd.DataFrame, max_rows: int = 10, max_cols: int = 10) -> Dict[str, Any]:
        """
        获取文件预览信息
    ...
    def save_temp_file(self, df: pd.DataFrame, file_type: str) -> Optional[str]:
        """
        保存临时文件
    ...
    def _save_uploaded_to_temp(self, uploaded_file) -> Optional[str]:
        """将上传文件保存到临时位置"""
        try:
            if self.temp_dir is None:
                self.temp_dir = tempfile.mkdtemp()
