...
# 导入本地工具
from .utils import DataValidator as BaseDataValidator, performance_monitor, EnhancedLogger
...
class DataValidator:
    ...
    def _validate_coefficient_matrices(self, processed_data: Dict[str, pd.DataFrame]) -> bool:
        """验证系数矩阵（w、a、b）"""
        try:
            matrices = ['w', 'a', 'b']
            ...
            for matrix_name in matrices:
                ...
                # 检查数值类型
                if not matrix.select_dtypes(include=[np.number]).shape[1] == matrix.shape[1]:
                    logger.error(f"{matrix_name}矩阵包含非数值列")
                    return False
                
                # 检查行索引是否为水质参数
                water_params_in_index = [param for param in self.standard_water_params 
                                       if param in matrix.index]
                if len(water_params_in_index) == 0:
                    logger.warning(f"{matrix_name}矩阵行索引中没有标准水质参数")
                
                # 检查列是否为特征
                stations_in_cols = [col for col in matrix.columns 
    ...
    def _validate_a_coefficient(self, a_data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            # 检查数值类型
            if not a_data.select_dtypes(include=[np.number]).shape[1] == a_data.shape[1]:
                logger.error("A系数包含非数值列")
                return False
            
            # 检查行索引
            water_params_in_index = [param for param in self.standard_water_params 
                                   if param in a_data.index]
            if len(water_params_in_index) == 0:
    ...
    def _validate_range_data(self, range_data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            # 检查列名是否包含水质参数
            water_params_in_cols = [col for col in range_data.columns 
                                  if col in self.standard_water_params]
    ...
    def check_data_consistency(self, processed_data: Dict[str, pd.DataFrame], model_type: int) -> Tuple[bool, List[str]]:
        """
        检查数据一致性
            ...
            if model_type == 1:
                ...
                if 'w' in processed_data and 'a' in processed_data:
                    if processed_data['w'].shape[1] != processed_data['a'].shape[1]:
                        warnings.append(f"w和a矩阵列数不一致: {processed_data['w'].shape[1]} vs {processed_data['a'].shape[1]}")
            
            # 检查A系数和其他矩阵的行数是否一致
            if 'A' in processed_data and model_type == 1:
                if 'w' in processed_data:
                    if processed_data['A'].shape[0] != processed_data['w'].shape[0]:
                        warnings.append(f"A系数和w矩阵行数不一致: {processed_data['A'].shape[0]} vs {processed_data['w'].shape[0]}")
    ...
    def _check_data_ranges(self, processed_data: Dict[str, pd.DataFrame], warnings: List[str]):
        """检查数据范围合理性"""
        try:
            # 检查系数矩阵的值范围
            for matrix_name in ['w', 'a', 'b']:
                if matrix_name in processed_data:
                    ...
                    if zero_ratio > 0.8:
                        warnings.append(f"{matrix_name}矩阵有{zero_ratio:.1%}的零值，可能需要检查")
                ...
                for param in range_data.columns:
                    if param in self.standard_water_params:
                        for value in range_data[param].dropna():
                            is_valid, error_msg = BaseDataValidator.validate_water_quality_indicator(value, param)
    ...
    def get_validation_report(self, processed_data: Dict[str, pd.DataFrame], model_type: int) -> Dict[str, Any]:
        """
        获取详细的验证报告
        ...
        try:
            # 格式验证
            report["validation_passed"] = self.validate_data_format(processed_data, model_type)
            
            # 一致性检查
            consistency_result, warnings = self.check_data_consistency(processed_data, model_type)
