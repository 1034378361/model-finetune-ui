...
class TestFullWorkflow:
    ...
    def test_type_0_full_workflow(self, sample_a_coefficient, sample_range_data, temp_dir):
        """测试Type 0完整工作流"""
        # 初始化组件
    ...
    def test_type_1_full_workflow(
        ...
        assert "a" in result
        assert "b" in result
        assert "A" in result  # Type 1自动生成A系数
        assert "Range" in result
    ...
    def test_template_generation_workflow(self):
        ...
        for template_type in type_0_templates:
            if template_type == "Range":
                ...
                template_content = generator.generate_coefficient_template(template_type)
